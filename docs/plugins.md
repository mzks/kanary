...
- 他の KANARY の `/export-alerts` を読みます
...
- `node_id` は既定では hostname を使います
- remote alert の `mirror_path` に自分の `node_id` が含まれている場合は import しません
...
- `origin_node_id`, `origin_rule_id`, `mirror_path` は payload に保持されます
